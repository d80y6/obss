...
export function useBillingJobs(filters: Record<string, string> = {}) {
  return useQuery({
    queryKey: queryKeys.billing.jobs.list(filters),
    queryFn: async () => {
      const params = new URLSearchParams()
      Object.entries(filters).forEach(([k, v]) => { if (v) params.set(k, v) })
      const res = await api.get(`/api/v1/billing/jobs?${params.toString()}`)
