...
export default function RefundPaymentPage() {
  ...
  return (
    <FormPageLayout title="Process Refund" backHref={`/payments/${id}`} onSubmit={handleSubmit((data) => mutation.mutate(
      { paymentId: id, reason: data.reason, amount: parseFloat(data.amount) },
      {
        onSuccess: () => {
