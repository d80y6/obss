namespace Obss.Orders.Application.DTOs;

public sealed record ProductOrderDto(
    Guid Id,
    string TenantId,
    string OrderNumber,
    Guid CustomerId,
    string CustomerName,
    DateTime OrderDate,
    string Status,
    string OrderType,
    decimal SubTotal,
    decimal TaxTotal,
    decimal DiscountTotal,
    decimal GrandTotal,
    string Currency,
    string? Notes,
    string? BillingAddressStreet,
    string? BillingAddressCity,
    string? BillingAddressState,
    string? BillingAddressPostalCode,
    string? BillingAddressCountry,
    string? ShippingAddressStreet,
    string? ShippingAddressCity,
    string? ShippingAddressState,
    string? ShippingAddressPostalCode,
    string? ShippingAddressCountry,
    string CreatedById,
    string? ApprovedById,
    DateTime? ApprovedAt,
    string? CancellationReason,
    string? Description,
    string? Channel,
    string? Priority,
    DateTime? RequestedStartDate,
    DateTime? RequestedCompletionDate,
    DateTime? ExpectedCompletionDate,
    string? NotificationContact,
    string? ExternalId,
    Guid? QuoteId,
    string? Href,
    string? AtType,
    string? AtBaseType,
    string? AtSchemaLocation,
    string? CompletionDate,
    DateTime CreatedAt,
    Guid? BillingAccountId,
    string? BillingAccountHref,
    int OrderVersion,
    string OrderPriority,
    Guid? ProductOfferingQualificationId,
    string? ProductOfferingQualificationHref,
    string? QuoteHref,
    List<ProductOrderItemDto> Items,
    List<RelatedPartyDto>? RelatedParties,
    List<ProductOrderPaymentDto> Payments,
    OrderFulfillmentDto? Fulfillment,
    List<ProductOrderMilestoneDto>? Milestones,
    List<ProductOrderItemRelationshipDto>? ItemRelationships);

public sealed record RelatedPartyDto(string PartyId, string PartyName, string Role);
