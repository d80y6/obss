...
public sealed class ActivateDiaCommandHandler : IRequestHandler<ActivateDiaCommand, Result<DiaLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ActivateDiaCommandHandler(
    ...
    public async Task<Result<DiaLifecycleResult>> Handle(ActivateDiaCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new DiaLifecycleResult(
            ...
            $"DIA service activation with {request.BandwidthMbps}Mbps bandwidth, {request.InterfaceType} handoff is in progress.",
            $"تفعيل خدمة الإنترنت المخصص بسعة {request.BandwidthMbps} ميجابت/ثانية ونوع التسليم {request.InterfaceType} قيد التنفيذ.",
            jobId));
    }
}
