...
public sealed class SuspendDiaCommandHandler : IRequestHandler<SuspendDiaCommand, Result<DiaLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendDiaCommandHandler(
    ...
    public async Task<Result<DiaLifecycleResult>> Handle(SuspendDiaCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DiaLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new DiaLifecycleResult(
            correlationId,
            "Suspended",
            $"DIA service suspended successfully. Reason: {request.Reason}",
            $"تم تعليق خدمة الإنترنت المخصص بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
