...
public sealed class ChangeDiaBandwidthCommandHandler : IRequestHandler<ChangeDiaBandwidthCommand, Result<DiaLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ChangeDiaBandwidthCommandHandler(
    ...
    public async Task<Result<DiaLifecycleResult>> Handle(ChangeDiaBandwidthCommand request, CancellationToken cancellationToken)
    {
        ...
        if (request.NewBandwidthMbps <= 0)
            return Result.Failure<DiaLifecycleResult>(Error.Validation("Bandwidth must be greater than zero."));
        ...
        return Result.Success(new DiaLifecycleResult(
            ...
            $"DIA bandwidth change to {request.NewBandwidthMbps}Mbps is in progress.",
            $"تغيير سعة الإنترنت المخصص إلى {request.NewBandwidthMbps} ميجابت/ثانية قيد التنفيذ.",
            jobId));
    }
}
