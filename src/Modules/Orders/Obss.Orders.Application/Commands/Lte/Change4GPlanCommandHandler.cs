...
public sealed class Change4GPlanCommandHandler : IRequestHandler<Change4GPlanCommand, Result<LifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public Change4GPlanCommandHandler(
    ...
    public async Task<Result<LifecycleResult>> Handle(Change4GPlanCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new LifecycleResult(
            ...
            $"4G plan change to {request.ApnName} ({request.DataAllowanceMb}MB, {request.ValidityDays} days) is in progress.",
            $"تغيير باقة 4G إلى {request.ApnName} ({request.DataAllowanceMb} ميجابايت، {request.ValidityDays} يوم) قيد التنفيذ.",
            jobId));
    }
}
