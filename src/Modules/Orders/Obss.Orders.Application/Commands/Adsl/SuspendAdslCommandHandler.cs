...
public sealed class SuspendAdslCommandHandler : IRequestHandler<SuspendAdslCommand, Result<AdslLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendAdslCommandHandler(
    ...
    public async Task<Result<AdslLifecycleResult>> Handle(SuspendAdslCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<AdslLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new AdslLifecycleResult(
            correlationId,
            "Suspended",
            $"ADSL service suspended successfully. Reason: {request.Reason}",
            $"تم تعليق خدمة ADSL بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
