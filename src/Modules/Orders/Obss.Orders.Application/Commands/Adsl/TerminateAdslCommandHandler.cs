...
public sealed class TerminateAdslCommandHandler : IRequestHandler<TerminateAdslCommand, Result<AdslLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public TerminateAdslCommandHandler(
    ...
    public async Task<Result<AdslLifecycleResult>> Handle(TerminateAdslCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<AdslLifecycleResult>(Error.Validation("Service is already inactive. Cannot terminate."));
        ...
        return Result.Success(new AdslLifecycleResult(
            ...
            $"ADSL service termination is in progress. Reason: {request.Reason}",
            $"إنهاء خدمة ADSL قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
