...
public sealed class ResumeDedicatedServerCommandHandler : IRequestHandler<ResumeDedicatedServerCommand, Result<DedicatedServerLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumeDedicatedServerCommandHandler(
    ...
    public async Task<Result<DedicatedServerLifecycleResult>> Handle(ResumeDedicatedServerCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new DedicatedServerLifecycleResult(
            correlationId,
            "Resumed",
            "Dedicated server resumed successfully.",
            "تم استئناف الخادم المخصص بنجاح.",
            jobId));
    }
}
