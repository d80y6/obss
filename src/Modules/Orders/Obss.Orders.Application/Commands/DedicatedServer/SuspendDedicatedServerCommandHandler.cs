...
public sealed class SuspendDedicatedServerCommandHandler : IRequestHandler<SuspendDedicatedServerCommand, Result<DedicatedServerLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendDedicatedServerCommandHandler(
    ...
    public async Task<Result<DedicatedServerLifecycleResult>> Handle(SuspendDedicatedServerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DedicatedServerLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new DedicatedServerLifecycleResult(
            correlationId,
            "Suspended",
            $"Dedicated server suspended successfully. Reason: {request.Reason}",
            $"تم تعليق الخادم المخصص بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
