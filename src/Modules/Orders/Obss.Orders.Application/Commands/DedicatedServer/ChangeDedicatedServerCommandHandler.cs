...
public sealed class ChangeDedicatedServerCommandHandler : IRequestHandler<ChangeDedicatedServerCommand, Result<DedicatedServerLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ChangeDedicatedServerCommandHandler(
    ...
    public async Task<Result<DedicatedServerLifecycleResult>> Handle(ChangeDedicatedServerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DedicatedServerLifecycleResult>(Error.Validation("Service is not active. Cannot change resources."));
        ...
        return Result.Success(new DedicatedServerLifecycleResult(
            correlationId,
            "ChangeInProgress",
            "Dedicated server resource change is in progress.",
            "تغيير موارد الخادم المخصص قيد التنفيذ.",
            jobId));
    }
}
