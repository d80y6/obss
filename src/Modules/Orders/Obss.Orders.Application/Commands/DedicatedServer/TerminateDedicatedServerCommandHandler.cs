...
public sealed class TerminateDedicatedServerCommandHandler : IRequestHandler<TerminateDedicatedServerCommand, Result<DedicatedServerLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public TerminateDedicatedServerCommandHandler(
    ...
    public async Task<Result<DedicatedServerLifecycleResult>> Handle(TerminateDedicatedServerCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DedicatedServerLifecycleResult>(Error.Validation("Service is already inactive. Cannot terminate."));
        ...
        return Result.Success(new DedicatedServerLifecycleResult(
            ...
            $"Dedicated server termination is in progress. Reason: {request.Reason}",
            $"إنهاء الخادم المخصص قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
