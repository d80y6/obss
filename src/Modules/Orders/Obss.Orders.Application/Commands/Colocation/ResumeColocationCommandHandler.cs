...
public sealed class ResumeColocationCommandHandler : IRequestHandler<ResumeColocationCommand, Result<ColocationLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumeColocationCommandHandler(
    ...
    public async Task<Result<ColocationLifecycleResult>> Handle(ResumeColocationCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new ColocationLifecycleResult(
            correlationId,
            "Resumed",
            "Colocation service resumed successfully.",
            "تم استئناف خدمة المساحة المشتركة بنجاح.",
            jobId));
    }
}
