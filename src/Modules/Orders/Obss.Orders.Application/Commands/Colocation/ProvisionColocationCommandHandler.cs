...
public sealed class ProvisionColocationCommandHandler : IRequestHandler<ProvisionColocationCommand, Result<ColocationLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ProvisionColocationCommandHandler(
    ...
    public async Task<Result<ColocationLifecycleResult>> Handle(ProvisionColocationCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new ColocationLifecycleResult(
            ...
            $"Colocation space provisioning is in progress. ({request.RackUnits}U, {request.PowerWatts}W, {request.CrossConnects} cross-connects)",
            $"توفير المساحة المشتركة قيد التنفيذ. ({request.RackUnits} وحدة، {request.PowerWatts} واط، {request.CrossConnects} وصلة)",
            jobId));
    }
}
