...
public sealed class SuspendColocationCommandHandler : IRequestHandler<SuspendColocationCommand, Result<ColocationLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendColocationCommandHandler(
    ...
    public async Task<Result<ColocationLifecycleResult>> Handle(SuspendColocationCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<ColocationLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new ColocationLifecycleResult(
            correlationId,
            "Suspended",
            $"Colocation service suspended successfully. Reason: {request.Reason}",
            $"تم تعليق خدمة المساحة المشتركة بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
