...
public sealed class SuspendHatifFawtaraCommandHandler : IRequestHandler<SuspendHatifFawtaraCommand, Result<HatifFawtaraLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendHatifFawtaraCommandHandler(
    ...
    public async Task<Result<HatifFawtaraLifecycleResult>> Handle(SuspendHatifFawtaraCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<HatifFawtaraLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new HatifFawtaraLifecycleResult(
            correlationId,
            "Suspended",
            $"Hatif Fawtara service suspended successfully. Reason: {request.Reason}",
            $"تم تعليق خدمة هاتفي فوترة بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
