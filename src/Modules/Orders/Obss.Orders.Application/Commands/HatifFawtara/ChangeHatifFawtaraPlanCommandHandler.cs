...
public sealed class ChangeHatifFawtaraPlanCommandHandler : IRequestHandler<ChangeHatifFawtaraPlanCommand, Result<HatifFawtaraLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ChangeHatifFawtaraPlanCommandHandler(
    ...
    public async Task<Result<HatifFawtaraLifecycleResult>> Handle(ChangeHatifFawtaraPlanCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new HatifFawtaraLifecycleResult(
            ...
            $"Hatif Fawtara plan change for {request.TelephoneNumber} is in progress.",
            $"تغيير باقة هاتفي فوترة للرقم {request.TelephoneNumber} قيد التنفيذ.",
            jobId));
    }
}
