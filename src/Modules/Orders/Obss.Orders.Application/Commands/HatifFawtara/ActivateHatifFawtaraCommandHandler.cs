...
public sealed class ActivateHatifFawtaraCommandHandler : IRequestHandler<ActivateHatifFawtaraCommand, Result<HatifFawtaraLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ActivateHatifFawtaraCommandHandler(
    ...
    public async Task<Result<HatifFawtaraLifecycleResult>> Handle(ActivateHatifFawtaraCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new HatifFawtaraLifecycleResult(
            correlationId,
            "Activated",
            $"Hatif Fawtara service activated for {request.TelephoneNumber}.",
            $"تم تفعيل خدمة هاتفي فوترة للرقم {request.TelephoneNumber}.",
            jobId));
    }
}
