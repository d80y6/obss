...
public sealed class ResumeTdmCircuitCommandHandler : IRequestHandler<ResumeTdmCircuitCommand, Result<TdmLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumeTdmCircuitCommandHandler(
    ...
    public async Task<Result<TdmLifecycleResult>> Handle(ResumeTdmCircuitCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new TdmLifecycleResult(
            correlationId,
            "Resumed",
            "TDM circuit resumed successfully.",
            "تم استئناف دائرة TDM بنجاح.",
            jobId));
    }
}
