...
public sealed class TerminateTdmCircuitCommandHandler : IRequestHandler<TerminateTdmCircuitCommand, Result<TdmLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public TerminateTdmCircuitCommandHandler(
    ...
    public async Task<Result<TdmLifecycleResult>> Handle(TerminateTdmCircuitCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<TdmLifecycleResult>(Error.Validation("Service is already inactive. Cannot terminate."));
        ...
        return Result.Success(new TdmLifecycleResult(
            ...
            $"TDM circuit termination is in progress. Reason: {request.Reason}",
            $"إنهاء دائرة TDM قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
