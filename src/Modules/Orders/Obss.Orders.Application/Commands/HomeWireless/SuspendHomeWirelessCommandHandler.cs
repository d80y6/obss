...
public sealed class SuspendHomeWirelessCommandHandler : IRequestHandler<SuspendHomeWirelessCommand, Result<HomeWirelessLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendHomeWirelessCommandHandler(
    ...
    public async Task<Result<HomeWirelessLifecycleResult>> Handle(SuspendHomeWirelessCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<HomeWirelessLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new HomeWirelessLifecycleResult(
            correlationId,
            "Suspended",
            $"Home Wireless 4G service suspended successfully. Reason: {request.Reason}",
            $"تم تعليق خدمة Home Wireless 4G بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
