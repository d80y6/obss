...
public sealed class ResumePriTrunkCommandHandler : IRequestHandler<ResumePriTrunkCommand, Result<PriLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumePriTrunkCommandHandler(
    ...
    public async Task<Result<PriLifecycleResult>> Handle(ResumePriTrunkCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new PriLifecycleResult(
            correlationId,
            "Resumed",
            "PRI trunk resumed successfully.",
            "تم استئناف قناة PRI بنجاح.",
            jobId));
    }
}
