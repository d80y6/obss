...
public sealed class ActivatePriTrunkCommandHandler : IRequestHandler<ActivatePriTrunkCommand, Result<PriLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ActivatePriTrunkCommandHandler(
    ...
    public async Task<Result<PriLifecycleResult>> Handle(ActivatePriTrunkCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new PriLifecycleResult(
            ...
            $"PRI trunk activation with {request.ChannelCount} channels, {request.SignalingProtocol} signaling is in progress.",
            $"تفعيل قناة PRI ب {request.ChannelCount} قناة وبروتوكول {request.SignalingProtocol} قيد التنفيذ.",
            jobId));
    }
}
