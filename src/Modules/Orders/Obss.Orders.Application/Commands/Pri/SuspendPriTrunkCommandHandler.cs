...
public sealed class SuspendPriTrunkCommandHandler : IRequestHandler<SuspendPriTrunkCommand, Result<PriLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendPriTrunkCommandHandler(
    ...
    public async Task<Result<PriLifecycleResult>> Handle(SuspendPriTrunkCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<PriLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new PriLifecycleResult(
            correlationId,
            "Suspended",
            $"PRI trunk suspended successfully. Reason: {request.Reason}",
            $"تم تعليق قناة PRI بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
