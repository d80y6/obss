...
public sealed class TerminateEthernetCommandHandler : IRequestHandler<TerminateEthernetCommand, Result<EthernetLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public TerminateEthernetCommandHandler(
    ...
    public async Task<Result<EthernetLifecycleResult>> Handle(TerminateEthernetCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<EthernetLifecycleResult>(Error.Validation("Service is already inactive. Cannot terminate."));
        ...
        return Result.Success(new EthernetLifecycleResult(
            ...
            $"Ethernet service termination is in progress. Reason: {request.Reason}",
            $"إنهاء خدمة الإيثرنت قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
