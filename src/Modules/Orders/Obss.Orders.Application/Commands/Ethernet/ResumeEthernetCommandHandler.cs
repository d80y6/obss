...
public sealed class ResumeEthernetCommandHandler : IRequestHandler<ResumeEthernetCommand, Result<EthernetLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumeEthernetCommandHandler(
    ...
    public async Task<Result<EthernetLifecycleResult>> Handle(ResumeEthernetCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new EthernetLifecycleResult(
            correlationId,
            "Resumed",
            "Ethernet service resumed successfully.",
            "تم استئناف خدمة الإيثرنت بنجاح.",
            jobId));
    }
}
