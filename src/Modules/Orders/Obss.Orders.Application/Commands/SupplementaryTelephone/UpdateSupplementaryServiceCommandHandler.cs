...
public sealed class UpdateSupplementaryServiceCommandHandler : IRequestHandler<UpdateSupplementaryServiceCommand, Result<SupplementaryServiceLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public UpdateSupplementaryServiceCommandHandler(
    ...
    public async Task<Result<SupplementaryServiceLifecycleResult>> Handle(UpdateSupplementaryServiceCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new SupplementaryServiceLifecycleResult(
            ...
            $"Supplementary service {request.ServiceFeature} update requires operator intervention. ZTE operations are blocked by vendor confirmation.",
            $"تحديث خدمة {request.ServiceFeature} الإضافية يتطلب تدخل المشغل. عمليات ZTE محظورة بانتظار تأكيد البائع.",
            jobId));
    }
}
