...
public sealed class SuspendAtmConnectionCommandHandler : IRequestHandler<SuspendAtmConnectionCommand, Result<AtmLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendAtmConnectionCommandHandler(
    ...
    public async Task<Result<AtmLifecycleResult>> Handle(SuspendAtmConnectionCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<AtmLifecycleResult>(Error.Validation("Connection is not active. Cannot suspend."));
        ...
        return Result.Success(new AtmLifecycleResult(
            correlationId,
            "Suspended",
            $"ATM connection suspended successfully. Reason: {request.Reason}",
            $"تم تعليق اتصال الصراف الآلي بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
