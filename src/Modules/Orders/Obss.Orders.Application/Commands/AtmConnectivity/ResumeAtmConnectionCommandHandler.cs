...
public sealed class ResumeAtmConnectionCommandHandler : IRequestHandler<ResumeAtmConnectionCommand, Result<AtmLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumeAtmConnectionCommandHandler(
    ...
    public async Task<Result<AtmLifecycleResult>> Handle(ResumeAtmConnectionCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new AtmLifecycleResult(
            correlationId,
            "Resumed",
            "ATM connection resumed successfully.",
            "تم استئناف اتصال الصراف الآلي بنجاح.",
            jobId));
    }
}
