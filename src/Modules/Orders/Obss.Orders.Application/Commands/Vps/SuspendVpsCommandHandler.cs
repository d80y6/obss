...
public sealed class SuspendVpsCommandHandler : IRequestHandler<SuspendVpsCommand, Result<VpsLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendVpsCommandHandler(
    ...
    public async Task<Result<VpsLifecycleResult>> Handle(SuspendVpsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<VpsLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new VpsLifecycleResult(
            correlationId,
            "Suspended",
            $"VPS suspended successfully. Reason: {request.Reason}",
            $"تم تعليق الخادم الافتراضي بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
