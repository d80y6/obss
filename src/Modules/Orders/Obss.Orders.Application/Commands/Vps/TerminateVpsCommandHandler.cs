...
public sealed class TerminateVpsCommandHandler : IRequestHandler<TerminateVpsCommand, Result<VpsLifecycleResult>>
{
    ...
    public async Task<Result<VpsLifecycleResult>> Handle(TerminateVpsCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<VpsLifecycleResult>(Error.Validation("Service is already inactive. Cannot terminate."));
        ...
        return Result.Success(new VpsLifecycleResult(
            ...
            $"VPS termination is in progress. Reason: {request.Reason}",
            $"إنهاء الخادم الافتراضي قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
