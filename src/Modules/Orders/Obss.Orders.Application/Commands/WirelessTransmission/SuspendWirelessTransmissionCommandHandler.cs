...
public sealed class SuspendWirelessTransmissionCommandHandler : IRequestHandler<SuspendWirelessTransmissionCommand, Result<WirelessTransmissionLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendWirelessTransmissionCommandHandler(
    ...
    public async Task<Result<WirelessTransmissionLifecycleResult>> Handle(SuspendWirelessTransmissionCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<WirelessTransmissionLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new WirelessTransmissionLifecycleResult(
            correlationId,
            "Suspended",
            $"Wireless transmission service suspended successfully. Reason: {request.Reason}",
            $"تم تعليق خدمة النقل اللاسلكي بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
