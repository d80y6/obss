...
public sealed class ChangeWirelessTransmissionCommandHandler : IRequestHandler<ChangeWirelessTransmissionCommand, Result<WirelessTransmissionLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ChangeWirelessTransmissionCommandHandler(
    ...
    public async Task<Result<WirelessTransmissionLifecycleResult>> Handle(ChangeWirelessTransmissionCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new WirelessTransmissionLifecycleResult(
            correlationId,
            "ChangeInProgress",
            "Wireless transmission configuration change is in progress.",
            "تغيير تكوين النقل اللاسلكي قيد التنفيذ.",
            jobId));
    }
}
