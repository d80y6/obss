...
public sealed class AllocateStaticIpCommandHandler : IRequestHandler<AllocateStaticIpCommand, Result<StaticIpLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public AllocateStaticIpCommandHandler(
    ...
    public async Task<Result<StaticIpLifecycleResult>> Handle(AllocateStaticIpCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new StaticIpLifecycleResult(
            ...
            $"Static IP allocation of {request.IpCount} {request.IpVersion} address(es) for {request.CustomerSite} is in progress.",
            $"تخصيص {request.IpCount} عنوان IP ثابت ({request.IpVersion}) للموقع {request.CustomerSite} قيد التنفيذ.",
            jobId));
    }
}
