...
public sealed class ActivateWifiAccessCommandHandler : IRequestHandler<ActivateWifiAccessCommand, Result<LifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ActivateWifiAccessCommandHandler(
    ...
    public async Task<Result<LifecycleResult>> Handle(ActivateWifiAccessCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new LifecycleResult(
            ...
            $"Yemen WiFi access activation is in progress for site {request.SiteName}.",
            $"تفعيل خدمة يمن واي فاي قيد التنفيذ للموقع {request.SiteNameAr}.",
            jobId,
            null));
    }
}
