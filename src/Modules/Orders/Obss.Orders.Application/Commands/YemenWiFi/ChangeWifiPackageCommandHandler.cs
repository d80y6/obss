...
public sealed class ChangeWifiPackageCommandHandler : IRequestHandler<ChangeWifiPackageCommand, Result<LifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ChangeWifiPackageCommandHandler(
    ...
    public async Task<Result<LifecycleResult>> Handle(ChangeWifiPackageCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new LifecycleResult(
            correlationId,
            "PackageChangeInProgress",
            $"Yemen WiFi package change to {request.PackageType} is in progress.",
            $"تغيير باقة يمن واي فاي إلى {request.PackageType} قيد التنفيذ.",
            jobId,
            null));
    }
}
