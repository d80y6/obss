...
public sealed class ResumeWebHostingCommandHandler : IRequestHandler<ResumeWebHostingCommand, Result<WebHostingLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public ResumeWebHostingCommandHandler(
    ...
    public async Task<Result<WebHostingLifecycleResult>> Handle(ResumeWebHostingCommand request, CancellationToken cancellationToken)
    {
        ...
        return Result.Success(new WebHostingLifecycleResult(
            correlationId,
            "Resumed",
            "Web hosting resumed successfully.",
            "تم استئناف استضافة المواقع بنجاح.",
            jobId));
    }
}
