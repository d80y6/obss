...
public sealed class SuspendWebHostingCommandHandler : IRequestHandler<SuspendWebHostingCommand, Result<WebHostingLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendWebHostingCommandHandler(
    ...
    public async Task<Result<WebHostingLifecycleResult>> Handle(SuspendWebHostingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<WebHostingLifecycleResult>(Error.Validation("Service is not active. Cannot suspend."));
        ...
        return Result.Success(new WebHostingLifecycleResult(
            correlationId,
            "Suspended",
            $"Web hosting suspended successfully. Reason: {request.Reason}",
            $"تم تعليق استضافة المواقع بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
