...
public sealed class TerminateWebHostingCommandHandler : IRequestHandler<TerminateWebHostingCommand, Result<WebHostingLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public TerminateWebHostingCommandHandler(
    ...
    public async Task<Result<WebHostingLifecycleResult>> Handle(TerminateWebHostingCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<WebHostingLifecycleResult>(Error.Validation("Service is already inactive. Cannot terminate."));
        ...
        return Result.Success(new WebHostingLifecycleResult(
            ...
            $"Web hosting termination is in progress. Reason: {request.Reason}",
            $"إنهاء استضافة المواقع قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
