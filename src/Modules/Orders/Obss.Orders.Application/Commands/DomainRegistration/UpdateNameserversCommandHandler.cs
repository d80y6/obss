...
public sealed class UpdateNameserversCommandHandler : IRequestHandler<UpdateNameserversCommand, Result<DomainLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public UpdateNameserversCommandHandler(
    ...
    public async Task<Result<DomainLifecycleResult>> Handle(UpdateNameserversCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DomainLifecycleResult>(Error.Validation("Domain is not active. Cannot update nameservers."));
        ...
        return Result.Success(new DomainLifecycleResult(
            correlationId,
            "NameserversUpdateInProgress",
            "Domain nameservers update is in progress.",
            "تحديث خوادم الأسماء للنطاق قيد التنفيذ.",
            jobId));
    }
}
