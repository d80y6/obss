...
public sealed class SuspendDomainCommandHandler : IRequestHandler<SuspendDomainCommand, Result<DomainLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public SuspendDomainCommandHandler(
    ...
    public async Task<Result<DomainLifecycleResult>> Handle(SuspendDomainCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DomainLifecycleResult>(Error.Validation("Domain is not active. Cannot suspend."));
        ...
        return Result.Success(new DomainLifecycleResult(
            correlationId,
            "Suspended",
            $"Domain suspended successfully. Reason: {request.Reason}",
            $"تم تعليق النطاق بنجاح. السبب: {request.Reason}",
            jobId));
    }
}
