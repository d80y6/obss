...
public sealed class TerminateDomainCommandHandler : IRequestHandler<TerminateDomainCommand, Result<DomainLifecycleResult>>
{
    private readonly IProductOrderRepository _orderRepository;
    private readonly IRepository<ProvisioningJob> _provisioningJobRepository;
    private readonly IUnitOfWork _unitOfWork;

    public TerminateDomainCommandHandler(
    ...
    public async Task<Result<DomainLifecycleResult>> Handle(TerminateDomainCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!item.IsActive)
            return Result.Failure<DomainLifecycleResult>(Error.Validation("Domain is already inactive. Cannot terminate."));
        ...
        return Result.Success(new DomainLifecycleResult(
            correlationId,
            "ReleaseInProgress",
            $"Domain release is in progress. Reason: {request.Reason}",
            $"إطلاق النطاق قيد التنفيذ. السبب: {request.Reason}",
            jobId));
    }
}
