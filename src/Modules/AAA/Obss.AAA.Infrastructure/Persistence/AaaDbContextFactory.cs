...
public class AaaDbContextFactory : IDesignTimeDbContextFactory<AaaDbContext>
{
    ...
    {
        var optionsBuilder = new DbContextOptionsBuilder<AaaDbContext>();
        var connectionString = args.Length > 0
            ? args[0]
            : "Host=localhost;Port=5432;Database=obss_aaa;Username=obss_admin;Password=obss_s3cur3_p@ss";

        optionsBuilder.UseNpgsql(connectionString, npgsqlBuilder =>
        {
            npgsqlBuilder.MigrationsAssembly(typeof(AaaDbContext).Assembly.FullName);
