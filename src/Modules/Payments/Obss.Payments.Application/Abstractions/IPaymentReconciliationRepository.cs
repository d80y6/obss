...
public interface IPaymentReconciliationRepository : IRepository<PaymentReconciliation>
{
    ...
    Task<IReadOnlyList<PaymentReconciliation>> GetFilteredAsync(string? status, DateTime? fromDate, DateTime? toDate, int page = 1, int pageSize = 20, CancellationToken cancellationToken = default);
